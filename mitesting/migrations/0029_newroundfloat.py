...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for q in orm.Question.objects.all():
            es_evalf=q.expression_set.exclude(n_digits=None).filter(round_decimals=None)
            es_round=q.expression_set.filter(n_digits=None).exclude(round_decimals=None)
            es_both=q.expression_set.exclude(n_digits=None).exclude(round_decimals=None)

            for expr in es_evalf:
                # find any answer options with this expression
                for ans in q.questionansweroption_set.filter(answer=expr.name):
                    ans.round_on_compare = expr.n_digits
                    ans.save()

                # add evalf filter to any template variable with just the expr

                search_pattern = r"{{ *%s *}}" % expr.name
                replace_pattern = r"{{ %s|evalf:%i }}" % (expr.name, expr.n_digits)

                modified=False

                if re.search(search_pattern, q.question_text):
                    q.question_text = re.sub(search_pattern, replace_pattern, 
                                             q.question_text)
                    modified=True


                if re.search(search_pattern, q.solution_text):
                    q.solution_text = re.sub(search_pattern, replace_pattern, 
                                             q.solution_text)
                    modified=True
                ...
                for sp in q.questionsubpart_set.all():
                    modified=False

                    if re.search(search_pattern, sp.question_text):
                        sp.question_text = re.sub(search_pattern, replace_pattern, 
                                                 sp.question_text)
                        modified=True


                    if re.search(search_pattern, sp.solution_text):
                        sp.solution_text = re.sub(search_pattern, replace_pattern, 
                                                 sp.solution_text)
                        modified=True
            ...
            for expr in es_round:
                # find any answer options with this expression
                for ans in q.questionansweroption_set.filter(answer=expr.name):
                    ans.round_on_compare = expr.round_decimals
                    ans.round_absolute = True
                    ans.save()

                # add round filter to any template variable with just the expr

                search_pattern = r"{{ *%s *}}" % expr.name
                replace_pattern = r"{{ %s|round:%i }}" % (expr.name, expr.round_decimals)
            ...
            for expr in es_both:
                ...
                # add evalf and round filters to any template variable with just the expr

                search_pattern = r"{{ *%s *}}" % expr.name
                replace_pattern = r"{{ %s|evalf:%i|round:%i }}" % (expr.name, expr.n_digits, expr.round_decimals)
    ...
    def backwards(self, orm):
        raise RuntimeError("Have not implemented backwards migration")
