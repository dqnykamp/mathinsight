...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for question in orm.Question.objects.all():
            for answer_option in question.questionansweroption_set.all():
                answer_option.answer_code = "multiple_choice"
                answer_option.answer_type \
                    = QuestionAnswerOption.MULTIPLE_CHOICE
                if answer_option.correct:
                    answer_option.percent_correct=100
                else:
                    answer_option.percent_correct=0
                answer_option.save()
            n=0
            for expression in question.expression_set.all():
                if not expression.split_symbols_on_compare \
                        or expression.normalize_on_compare:
                    n+=1;
                    question.questionansweroption_set.create(
                        answer_code = expression.name,
                        answer = expression.name,
                        percent_correct = 100.0,
                        split_symbols_on_compare 
                        = expression.split_symbols_on_compare,
                        normalize_on_compare = expression.normalize_on_compare,
                        sort_order=n)
