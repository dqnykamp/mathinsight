...
def _render_question(question_code, context, blank_style=False):

    # test if question with question_code exists
    try:
        thequestion=Question.objects.get(code=question_code)
    # if question does not exist, mark as broken. 
        ...
        if(blank_style):
            ...
            return "<p>[Broken Question]</p>"
            
    answer_form = QuestionForm(prefix=question_code)
    
    answer_form.fields['answers'].queryset = thequestion.questionansweroption_set.order_by('?')

    send_command = "Dajaxice.midocs.send_form(callback_%s,{'form':$('#question_%s').serializeObject(), 'prefix':'%s'})" % (thequestion.code, thequestion.code, thequestion.code)

    callback_script = '<script type="text/javascript">function callback_%s(data){Dajax.process(data); MathJax.Hub.Queue(["Typeset",MathJax.Hub,"question_%s_feedback"]);}</script>' % (thequestion.code, thequestion.code)


    html_string = '%s<div class="question"><p>%s</p><form action="" method="post" id="question_%s" >' % (callback_script, thequestion.question, thequestion.code)
    #html_string += template.Template("{% csrf_token %}").render(context)

    for field in answer_form:
        if field.html_name == "%s-answers" % thequestion.code:
    ...
    html_string = '%s<div id="question_%s_feedback" class="info"></div><p><input type="button" value="Submit" onclick="%s"></p></form></div>'  % (html_string, thequestion.code, send_command)
...
class QuestionNode(template.Node):
    ...
    def render(self, context):
        ...
        return _render_question(self.question_code, context, blank_style)
...
def display_question(parser, token):
    ...
    question_code=bits[1]
  
    if not (question_code[0] == question_code[-1] and question_code[0] in ('"', "'")):
        raise template.TemplateSyntaxError, "%r tag's argument should be in quotes" % bits[0]
    question_code = question_code[1:-1]

    return QuestionNode(question_code)
...
class VideoQuestionsNode(template.Node):
    ...
    def render(self, context):
        ...
        try:
            thevideo=Video.objects.get(code=self.video_code)
        ...
        for question in thevideo.question_set.all():
            html_string = html_string + _render_question(question.code, context, blank_style)
        return html_string
...
def display_video_questions(parser, token):
    ...
    if not (video_code[0] == video_code[-1] and video_code[0] in ('"', "'")):
        raise template.TemplateSyntaxError, "%r tag's argument should be in quotes" % bits[0]
    video_code = video_code[1:-1]

    return VideoQuestionsNode(video_code)
